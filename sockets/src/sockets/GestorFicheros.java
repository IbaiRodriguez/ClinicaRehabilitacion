...
public class GestorFicheros {
	...
	public static void inicializarListaEjercicios(String tipoLesion, DataOutputStream out) {
		BufferedReader in = null;
		String lineaLeida = null;
		...
		try {
			in = new BufferedReader(new FileReader(NOMBRE_FICH_LESIONES));
			while((lineaLeida = in.readLine()) != null){
				valoresEjercicio = lineaLeida.split(SEPARADOR_DOLAR);
				
				if(valoresEjercicio[NOMBRE_LESION].equals(tipoLesion)){
					ejerciciosAHacer = valoresEjercicio[ID_DE_EJERCICIOS].split(SEPARADOR_NUMEROS);
					
					for(String ejercicio : ejerciciosAHacer){
						idDeEjercicios.add(ejercicio);
					}
					break;
				}
